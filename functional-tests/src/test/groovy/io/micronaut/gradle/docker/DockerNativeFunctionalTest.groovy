...
"""
        ...
        then:
        dockerFile.readLines().findAll { it.contains('/config-dirs') }.every { it.contains('/home/alternate/config-dirs') }
...
"""
        ...
        dockerfileNativeLines.find { it.startsWith('RUN native-image ') }.contains(SHARED_ARENA_SUPPORT)
        ...
        withoutReachabilityMetadataConfigLines(dockerfileNative) == """FROM ghcr.io/graalvm/native-image-community:25-ol${DefaultVersions.ORACLELINUX} AS graalvm
WORKDIR /home/app
COPY --link layers/libs /home/app/libs
COPY --link server.iprof /home/app/server.iprof
COPY --link layers/app /home/app/
