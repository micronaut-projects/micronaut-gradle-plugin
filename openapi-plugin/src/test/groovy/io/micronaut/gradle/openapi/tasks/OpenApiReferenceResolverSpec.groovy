...
class OpenApiReferenceResolverSpec extends Specification {
    ...
    private final List<File> parserClasspath = System.getProperty("java.class.path")
    ...
    def "recursively resolves local refs and ignores remote or document-only refs"() {
        ...
        when:
        def referencedFiles = OpenApiReferenceResolver.referencedFiles(root, parserClasspath)*.canonicalFile as Set
    ...
    def "parses referenced documents according to json or yaml file type"() {
        given:
        def root = temporaryFolder.newFile("openapi.yml")
        def jsonSchema = temporaryFolder.newFile("request.JSON")
        def yamlSchema = temporaryFolder.newFile("shared.yaml")
        ...
        jsonSchema.text = '''
            {
              "$defs": {
                "Req": {
                  "$ref": "./shared.yaml#/components/schemas/Shared"
                }
              }
            }
        '''.stripIndent()
        ...
        then:
        referencedFiles == [jsonSchema.canonicalFile, yamlSchema.canonicalFile] as Set
    }

    def "does not parse yaml syntax from json files"() {
        given:
        def root = temporaryFolder.newFile("openapi.yml")
        def jsonSchema = temporaryFolder.newFile("request.json")
        def yamlOnlyReference = temporaryFolder.newFile("yaml-only.json")
                ...
                post:
                  ...
                  responses:
                    "200":
                      description: ok
        """.stripIndent()
        jsonSchema.text = "\$ref: './yaml-only.json'"
        yamlOnlyReference.text = '{}'

        when:
        def referencedFiles = OpenApiReferenceResolver.referencedFiles(root, parserClasspath)*.canonicalFile as Set

        then:
        referencedFiles == [jsonSchema.canonicalFile] as Set
        !referencedFiles.contains(yamlOnlyReference.canonicalFile)
    }
    ...
    def "uses safe yaml loading for tagged values"() {
        ...
        when:
        def referencedFiles = OpenApiReferenceResolver.referencedFiles(root, parserClasspath)
