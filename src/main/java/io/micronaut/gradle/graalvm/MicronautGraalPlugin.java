...
public class MicronautGraalPlugin implements Plugin<Project> {
    ...
    public void apply(Project project) {
        ...
        findExtensionsByType(NativeImageOptions.class, project).forEach(options ->
                options.resources(rsrc -> rsrc.inference(inf -> {
                    inf.getEnabled().convention(true);
                }))
        );
        project.getPluginManager().withPlugin("application", plugin -> {
            TaskContainer tasks = project.getTasks();
            tasks.withType(BuildNativeImageTask.class).named("nativeBuild", nativeImageTask -> {
    ...
    private void registerTestAgainstNativeImageTask(Project project, TaskContainer tasks, String testName) {
        tasks.register(testName + "NativeImage", Test.class, nativeImageTestTask -> {
            ...
            BuildNativeImageTask nativeBuild = (BuildNativeImageTask) tasks.findByName("nativeBuild");
    ...
    private static <T> Set<T> findExtensionsByType(Class<T> type, Project project) {
        Set<T> extensions = new HashSet<>();
        ExtensionContainer container = project.getExtensions();
        container.getExtensionsSchema().forEach(schema -> {
            if (type.isAssignableFrom(schema.getPublicType().getConcreteClass())) {
                //noinspection unchecked
                extensions.add((T) container.findByName(schema.getName()));
            }
        });
        return extensions;
    }
