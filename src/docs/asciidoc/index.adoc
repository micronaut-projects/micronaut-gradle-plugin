...
micronaut {
    testResources {
        ...
        testTasks.add("integrationTest") // empty by default, applies to all Test tasks when unset
...
- by default, test resources support is wired into all Gradle `Test` tasks. Set the `testTasks` property to a non-empty set of task names, such as `integrationTest`, to restrict wiring to those specific `Test` tasks.
...
[NOTE]
====
This task selection only narrows Gradle `Test` task wiring. `JavaExec` task wiring is unchanged.
====
