...
- the https://plugins.gradle.org/plugin/io.micronaut.function[`io.micronaut.function`] plugin is aimed at building Micronaut functions. It keeps the same Docker and GraalVM integration as the application plugin while preserving function-specific main-class handling.
...
|https://plugins.gradle.org/plugin/io.micronaut.minimal.function[`io.micronaut.minimal.function`]
|Allows building Micronaut functions, without GraalVM nor Docker support
|`java`; auto-applies `application` only when the selected runtime requires application-oriented packaging (for example `lambda_provided`)
...
|https://plugins.gradle.org/plugin/io.micronaut.function[`io.micronaut.function`]
|A typical Micronaut Function, with support for GraalVM and Docker
|https://plugins.gradle.org/plugin/io.micronaut.minimal.function[`io.micronaut.minimal.function`], https://plugins.gradle.org/plugin/io.micronaut.graalvm[`io.micronaut.graalvm`], https://plugins.gradle.org/plugin/io.micronaut.docker[`io.micronaut.docker`], `AptEclipsePlugin`
...
Use `io.micronaut.function` or `io.micronaut.minimal.function` for function-oriented projects generated by Micronaut Launch or the CLI.
Those plugin ids keep the same runtime dependency resolution as the application plugins, but they only auto-apply Gradle's `application` plugin when the selected runtime needs application-oriented packaging support.

For `lambda_provided`, the function plugins auto-apply the `application` plugin and keep the configured function main class for runner JAR and native-image packaging.
Use the Micronaut function extension as the supported DSL-neutral configuration path:

[source, groovy, subs="verbatim,attributes", role="multi-language-sample"]
----
micronaut {
    function {
        mainClass = "example.Function"
    }
}
----
...
micronaut {
    function {
        mainClass.set("example.Function")
    }
}
----

In Groovy DSL builds that already rely on Gradle's `application` extension, `application { mainClass = "example.Function" }` continues to work for `lambda_provided`.
For `lambda_java`, the function plugins do not auto-apply the `application` plugin.
...
* `lambda_java` - Allows building the application or function into an AWS Lambda on the managed Java runtime
* `lambda_provided` - Allows building the application or function into an AWS Lambda custom runtime
...
If you are interested in deploying your Micronaut application to AWS Lambda using GraalVM, set the runtime to `lambda_provided` and execute `./gradlew buildNativeLambda`.
...
When you are building a function project, prefer the `io.micronaut.function` or `io.micronaut.minimal.function` plugin ids and configure `micronaut.function.mainClass` with your function entry point.
For function builds on `lambda_provided`, the configured function main class is preserved for native images.
If you are using the Groovy DSL and already rely on Gradle's `application {}` block, that path remains supported for `lambda_provided`.
