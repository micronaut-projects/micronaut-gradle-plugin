...
micronaut {
    version.set("{micronaut-version}")
    runtime.set("netty")
    serialization.set("serde-jackson")
}
...
The plugin also supports selecting the runtime serialization library. By default, minimal application builds use `serde-jackson`, which adds the runtime-only dependency `io.micronaut.serde:micronaut-serde-jackson`. You can switch to `jackson` to add `io.micronaut.jackson:micronaut-jackson-databind` instead:
