...
When you are building a function project, prefer the `io.micronaut.function` or `io.micronaut.minimal.function` plugin ids and configure `application.mainClass` with your function entry point.
For function builds on `lambda_provided`, the configured main class is preserved for native images.
